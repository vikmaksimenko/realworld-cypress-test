...
describe('user creating articles', () => {
  ...
  it('can create 11 new articles', () => {
    ...
    cy.get('[name=title]').should('be.visible').type('Article1')
    cy.get('[name=description]').type('Description1')
    cy.get('[name=body]').type('Body1')
    cy.get('[name=tagList]').type('tag1,tag2')
    cy.get('[type=submit]').click()

    cy.contains('Edit Article').should('be.visible')
    cy.get('.banner h1').should('have.text', 'Article1')
    cy.get('.article-content > div > div').
        first().should('have.text', 'Description1').
        next().should('contain.text', 'Body1')
